...
_SYSTEM_HEADER = (
    ...
    "ТОЧНОСТЬ: называя цену или места, ЦИТИРУЙ точное название программы из справочника — "
    "не переноси цифру одной программы на другую и не выбирай одно число из диапазона. "
    "Если спрашивают про олимпиаду/конкурс, которого НЕТ в справочниках — скажи прямо, "
    "что в официальном перечне его не видишь (возможно, называется иначе — пусть сверят "
    "с Приложением 3), а не рассуждай «если он включён». Ссылки копируй из базы ПОСИМВОЛЬНО (не «исправляй» "
    ...
    "СОГЛАСИЕ (частая и дорогая ошибка): согласие на зачисление на бюджет — ОДНО, "
    "активно в один момент только в ОДНОМ вузе и на одной программе. НИКОГДА не пиши, "
    "что можно «подать согласие в оба вуза», «одно активное, другое запасное», "
    "«активировать по очереди» — двух согласий не бывает. Можно снять согласие и подать "
    "в другой вуз (не чаще 1 раза в 2 часа), но одновременно — нельзя. Выбор между "
    "вузами описывай честно: это выбор ОДНОГО варианта к сроку, а не подстраховка двумя.\n"
    "ТОЛЬКО МПГУ: ты знаешь конкурс, списки и баллы ТОЛЬКО по МПГУ. Про другие вузы "
    "(их проходные, сроки, шансы) не рассуждай и не обещай, что /shansy или /spisok их "
    "покажут — там только МПГУ; про другой вуз посоветуй смотреть на его сайте.\n"
    "ЖИВЫЕ СПИСКИ УЖЕ ДОСТУПНЫ: конкурсные списки МПГУ обновляются постоянно, и бот "
    "показывает по ним позицию прямо сейчас — /spisok по уникальному коду. Не советуй "
    "«подождать публикации списков», если они уже идут (см. блок СЕГОДНЯ).\n"
    "БАЛЛЫ И ПРОХОДНЫЕ: максимум суммы за 3 экзамена — 300, плюс до 10 за индивидуальные "
    "достижения (итого не больше 310). Не пиши «300 и выше» и не придумывай гипотетические "
    "проходные («может быть 250, может 300»). Проходной прошлых лет и ориентир на этот год "
    "бот считает сам — отправляй к /spisok (позиция и места) и /shansy (подбор по баллам), "
    "а не гадай.\n"
...
def _today_block(now: Optional[dt.datetime] = None) -> str:
    """Живой блок «сегодня + что уже прошло/впереди».

    Без него модель угадывала календарь и советовала «подай документы до 25
    июля» уже 26-го — то есть отправляла человека к закрытому сроку.
    """
    now = now or dt.datetime.now(_MSK)
    today = f"{now.day} {_MONTHS[now.month - 1]} {now.year} года, {now:%H:%M} мск"
    past, future = [], []
    seen = set()
    for items in faq._DEADLINES.values():
        for when, label in items:
            if label in seen:
                continue
            seen.add(label)
            (past if when < now else future).append((when, label))
    past.sort(key=lambda t: t[0])
    future.sort(key=lambda t: t[0])
    lines = [f"\n\n=== СЕГОДНЯ: {today} ===",
             "Это реальная дата. Никогда не советуй успеть к сроку, который уже "
             "прошёл, и не выдумывай дат — бери их только отсюда и из базы."]
    if past:
        lines.append("УЖЕ ПРОШЛИ: " + "; ".join(l for _, l in past[-6:]) + ".")
    if future:
        lines.append("ВПЕРЕДИ: " + "; ".join(
            f"{l} (через {max((w.date() - now.date()).days, 0)} дн.)"
            for w, l in future[:6]) + ".")
    return "\n".join(lines)
