...
def test_short_card_shows_both_the_list_place_and_the_consent_estimate():
    ...
    Вопрос реальный, цифры в тесте — синтетические. Живой случай (код
    1973903, магистратура): место в списке 106, а среди подавших согласие и
    реально борющихся за места — 60-е, потому что 17 человек выше проходят
    по своему более высокому приоритету в другом месте. Пока в карточке
    стояло одно число, понять, которое из них правда, было нельзя.
    """
    from scraper.abitur.lists import format_positions_short
